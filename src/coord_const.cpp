...
struct ConstantCoord1D : public CoordNode
{
	...
	if (initialized_by_coord) {
	    assert(dim<pos.output.h_ptr()->row_width);
            traverse_dset<1,int> (grp, "id", [&](size_t i, int x) {params[i].id = x;});
	}
	else{
            n_elem = get_dset_size(1, grp, "value")[0];
            check_size(grp, "value", n_elem, 1);
            traverse_dset<1,float>(grp, "value", [&](size_t ne, float x) { const_cast<VecArrayStorage&>(*output.h_ptr())(0,ne) = x;});
	}
	...
	if (initialized_by_coord and not initialized) {
            ...
            for(int nt=0; nt<n_elem; ++nt) {
                auto& p = params[nt];
	        const_cast<VecArrayStorage&>(*output.h_ptr())(0, nt) = posc(dim, p.id);
            }
	    initialized = true;
	}
    }
...
struct ConstantCoord2D : public CoordNode
{
    ...
    {
	if (initialized_by_coord) {
	    assert(dim1<pos.output.h_ptr()->row_width);
	    assert(dim2<pos.output.h_ptr()->row_width);
            traverse_dset<1,int> (grp, "id", [&](size_t i, int x) {params[i].id = x;});
	}
	else{
            n_elem = get_dset_size(2, grp, "value")[0];
            check_size(grp, "value", n_elem, 2);
            traverse_dset<2,float>(grp, "value", [&](size_t ne, size_t nt, float x) { const_cast<VecArrayStorage&>(*output.h_ptr())(nt, ne) = x;});
	}
    }
	...
	if (initialized_by_coord and not initialized) {
            ...
            for(int nt=0; nt<n_elem; ++nt) {
                auto& p = params[nt];
	        const_cast<VecArrayStorage&>(*output.h_ptr())(0, nt) = posc(dim1, p.id);
	        const_cast<VecArrayStorage&>(*output.h_ptr())(1, nt) = posc(dim2, p.id);
            }
	    initialized = true;
	}
...
struct ConstantCoord3D : public CoordNode
{
	...
	if (initialized_by_coord) {
	    assert(dim1<pos.output.h_ptr()->row_width);
	    assert(dim2<pos.output.h_ptr()->row_width);
	    assert(dim3<pos.output.h_ptr()->row_width);
            traverse_dset<1,int> (grp, "id", [&](size_t i, int x) {params[i].id = x;});
	}
	else{
            n_elem = get_dset_size(2, grp, "value")[0];
        ...
        if(logging(LOG_DETAILED)) {
            if (default_logger) {
                ...
                default_logger->add_logger<float>( ("Const3D_" + to_string(log_suffix)).c_str(), {n_elem, 3}, [&](float* buffer) {
                    for(int nt=0; nt<n_elem; ++nt) {
                        auto x = load_vec<3>(const_cast<VecArrayStorage&>(*output.h_ptr()), nt);
	...
	if (initialized_by_coord and not initialized) {
	        ...
	        const_cast<VecArrayStorage&>(*output.h_ptr())(0, nt) = posc(dim1, p.id);
	        const_cast<VecArrayStorage&>(*output.h_ptr())(1, nt) = posc(dim2, p.id);
	        const_cast<VecArrayStorage&>(*output.h_ptr())(2, nt) = posc(dim3, p.id);
            }
	    initialized = true;
	}
    }
...
struct MovingConstantCoord1D : public CoordNode
{
	...
	if (initialized_by_coord) {
	    ...
	    assert(dim<pos.output.h_ptr()->row_width);
            ...
            traverse_dset<1,float>(grp, "start_pos", [&](size_t i, float x) { const_cast<VecArrayStorage&>(*output.h_ptr())(0,i) = x; p[i].start_pos = x;});
	}
	...
	if (initialized_by_coord and not initialized) {
            ...
            for(int nt=0; nt<n_elem; ++nt) {
                auto& pa = params[nt];
	        const_cast<VecArrayStorage&>(*output.h_ptr())(0, nt) = posc(dim, pa.id);
        ...
        for(auto &p: params) {
            const_cast<VecArrayStorage&>(*output.h_ptr())(0, p.id) = p.start_pos + p.velocities * time_estimate;
        }
    }
    virtual void propagate_deriv() {}
};
...
struct WhirlingConstantCoord1D : public CoordNode
{
	...
	if (initialized_by_coord) {
	    ...
	    assert(dim<pos.output.h_ptr()->row_width);
            ...
            traverse_dset<1,float>(grp, "start_angle", [&](size_t i, float x) { const_cast<VecArrayStorage&>(*output.h_ptr())(0,i) = x; p[i].start_angle = x;});
	}
	...
	if (initialized_by_coord and not initialized) {
	    ...
	    float angle = p.start_angle + p.whirling_vel * time_estimate;
            ...
            if (angle < -M_PI_F)
                angle += 2*M_PI_F;
            const_cast<VecArrayStorage&>(*output.h_ptr())(0, p.id) = angle;
        }
    }
    virtual void propagate_deriv() {}
};
...
struct MovingConstantCoord2D : public CoordNode
{
	...
	if (initialized_by_coord) {
	    ...
	    assert(dim2<pos.output.h_ptr()->row_width);
            ...
            traverse_dset<2,float>(grp, "start_pos", [&](size_t i, size_t j, float x) { const_cast<VecArrayStorage&>(*output.h_ptr())(j,i) = x; p[i].start_pos[j] = x;});
	}
	...
	if (initialized_by_coord and not initialized) {
            ...
            for(int nt=0; nt<n_elem; ++nt) {
                auto& pa = params[nt];
	        const_cast<VecArrayStorage&>(*output.h_ptr())(0, nt)  = posc(dim1, pa.id);
	        const_cast<VecArrayStorage&>(*output.h_ptr())(1, nt)  = posc(dim2, pa.id);
        ...
        for(auto &p: params) {
	    for(int i: range(2)) {
                const_cast<VecArrayStorage&>(*output.h_ptr())(i, p.id) = p.start_pos[i] + p.velocities[i] * time_estimate;
	    }
        }
    }
    virtual void propagate_deriv() {}
...
struct MovingConstantCoord3D : public CoordNode
{
	...
	if (initialized_by_coord and not initialized) {
	        ...
	        const_cast<VecArrayStorage&>(*output.h_ptr())(0, nt)   = posc(dim1, pa.id);
	        const_cast<VecArrayStorage&>(*output.h_ptr())(1, nt)   = posc(dim2, pa.id);
	        const_cast<VecArrayStorage&>(*output.h_ptr())(2, nt)   = posc(dim3, pa.id);
		pa.start_pos[0] = posc(dim1, pa.id);
		pa.start_pos[1] = posc(dim2, pa.id);
        ...
        for(int nt=0; nt<n_elem; ++nt) {
            auto& pa = params[nt];
	    for(int i: range(3)) {
                const_cast<VecArrayStorage&>(*output.h_ptr())(i, nt) = pa.start_pos[i] + pa.velocities[i] * time_estimate;
	    }
        }
    }
    virtual void propagate_deriv() {}
