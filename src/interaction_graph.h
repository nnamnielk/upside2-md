...
struct InteractionGraph{
    ...
    void compute_edges() {
        ...
        {
            VecArray posv(pos_node1->output.get_mutable_host_ptr(), pos_node1->elem_width);
            for(int ne=0; ne<n_elem1; ++ne) 
                store_vec(pos1.get()+ne*n_dim1a, load_vec<n_dim1>(posv, loc1[ne]));
        }
        if(!symmetric) {
            VecArray posv(pos_node2->output.get_mutable_host_ptr(), pos_node2->elem_width);
    ...
    void propagate_derivatives() {
        ...
        {
            VecArray pos1_sens(pos_node1->sens.get_mutable_host_ptr(), pos_node1->elem_width);
            for(int i1=0; i1<n_elem1; ++i1)
                update_vec(pos1_sens, loc1[i1], load_vec<n_dim1>(pos1_deriv+i1*n_dim1a));
        }
        if(!symmetric) {
            VecArray pos2_sens(pos_node2->sens.get_mutable_host_ptr(), pos_node2->elem_width);
