...
struct DistCoord : public CoordNode
{
    ...
    virtual void compute_value(ComputeMode mode) {
        ...
        VecArray posc1(pos1.output.get_mutable_host_ptr(), pos1.elem_width);
        VecArray posc2(pos2.output.get_mutable_host_ptr(), pos2.elem_width);
        for(int nt=0; nt<n_elem; ++nt) {
            ...
            auto disp = x1 - x2;
	    auto dist = mag(disp);
	    output.get_mutable_host_ptr()[nt] = dist;
	    if (dist != 0.)
    ...
    virtual void propagate_deriv() {
        ...
        VecArray pos_sens1(pos1.sens.get_mutable_host_ptr(), pos1.elem_width);
        VecArray pos_sens2(pos2.sens.get_mutable_host_ptr(), pos2.elem_width);
        for(int nt=0; nt<n_elem; ++nt) {
            auto& p = params[nt];
            auto  d = sens.get_host_ptr()[nt];
...
struct Dist2DCoord : public CoordNode
{
    ...
    virtual void compute_value(ComputeMode mode) {
        ...
        for(int nt=0; nt<n_elem; ++nt) {
            auto& p       = params[nt];
            auto x1       = pos1.output.get_mutable_host_ptr()[p.atom[0] * pos1.elem_width + dim1];
            auto x2       = pos2.output.get_mutable_host_ptr()[p.atom[1] * pos2.elem_width + dim1];
            auto y1       = pos1.output.get_mutable_host_ptr()[p.atom[0] * pos1.elem_width + dim2];
            auto y2       = pos2.output.get_mutable_host_ptr()[p.atom[1] * pos2.elem_width + dim2];
    ...
    virtual void propagate_deriv() {
        ...
        for(int nt=0; nt<n_elem; ++nt) {
            auto& p = params[nt];
            auto  d = sens.get_host_ptr()[nt];
            pos1.sens.get_mutable_host_ptr()[p.atom[0] * pos1.elem_width + dim1] += deriv[nt][0]*d;
            pos2.sens.get_mutable_host_ptr()[p.atom[1] * pos2.elem_width + dim1] -= deriv[nt][0]*d;
            pos1.sens.get_mutable_host_ptr()[p.atom[0] * pos1.elem_width + dim2] += deriv[nt][1]*d;
            pos2.sens.get_mutable_host_ptr()[p.atom[1] * pos2.elem_width + dim2] -= deriv[nt][1]*d;
...
struct Dist1DCoord : public CoordNode
{
    ...
    virtual void propagate_deriv() {
        ...
        for(int nt=0; nt<n_elem; ++nt) {
            auto& p = params[nt];
            auto  d = sens.get_host_ptr()[nt];
            pos1.sens.get_mutable_host_ptr()[p.atom[0] * pos1.elem_width + dim1] += d*deriv[nt];
            pos2.sens.get_mutable_host_ptr()[p.atom[1] * pos2.elem_width + dim1] -= d*deriv[nt];
...
struct AngleCoord : public CoordNode
{
    ...
    virtual void compute_value(ComputeMode mode) {
        ...
        for(int nt=0; nt<n_elem; ++nt) {
            ...
            auto dp = dot(x1h, x2h);
	    output.get_mutable_host_ptr()[nt] = dp;
    ...
    virtual void propagate_deriv() {
        ...
        for(int nt=0; nt<n_elem; ++nt) {
            auto& p = params[nt];
            update_vec(pos_sens, p.atom[0], sens.get_host_ptr()[nt]*deriv1[nt]);
            update_vec(pos_sens, p.atom[1], sens.get_host_ptr()[nt]*deriv2[nt]);
            update_vec(pos_sens, p.atom[2], sens.get_host_ptr()[nt]*deriv3[nt]);
        }
    }
};
...
struct AngleCoord2 : public CoordNode
{
    ...
    virtual void propagate_deriv() {
        ...
        for(int nt=0; nt<n_elem; ++nt) {
            ...
            update_vec(pos_sens, p.atom[1], sens.get_host_ptr()[nt]*deriv2[nt]);
            update_vec(pos_sens, p.atom[2], sens.get_host_ptr()[nt]*deriv3[nt]);
            update_vec(pos_sens, p.atom[3], sens.get_host_ptr()[nt]*deriv4[nt]);
        }
    }
};
...
struct DihedralCoord : public CoordNode
{
    ...
    virtual void compute_value(ComputeMode mode) {
        ...
        VecArray dihe_pos(output.get_mutable_host_ptr(), elem_width);
        float*   posv     = pos.output.get_mutable_host_ptr();
    ...
    virtual void propagate_deriv() {
	    ...
	    if (p.dummy_angle)
            ...
            Float4 ps[4];
            for(int na: range(4))
                ps[na] = fmadd( Float4(sens.get_host_ptr()[nt]), Float4(jac[nt].j[na]), Float4(pos_sens + 4*p.atom[na]) );
...
struct PosCenter : public CoordNode {
    ...
    virtual void compute_value(ComputeMode mode) override {
        ...
        for(int nd=0; nd<n_dim; ++nd) {
            for(int g=0; g<n_group; ++g) {
                output.get_mutable_host_ptr()[g * elem_width + nd] = 0.0;
                for(int na=border[g]; na<border[g+1]; ++na) {
                    output.get_mutable_host_ptr()[g * elem_width + nd] += pos.output.get_mutable_host_ptr()[index_pos[na] * pos.elem_width + index_dim[nd]] * scale[g];
                }
            }
        }
    }
    ...
    virtual void propagate_deriv() override {
        ...
        for(int nd=0; nd<n_dim; ++nd) {
            for(int g=0; g<n_group; ++g) {
                auto scale_sens = sens.get_host_ptr()[g * elem_width + nd]*scale[g];
                for(int na=border[g]; na<border[g+1]; ++na) {
                    pos.sens.get_mutable_host_ptr()[index_pos[na] * pos.elem_width + index_dim[nd]] += scale_sens;
                }
            }
        }
    }
